...
# https://github.com/atoptima/Coluna.jl/blob/v0.8.0/src/ColGen/ColGen.jl
# Original code is licensed under MPL 2.0
# Copyright (c) Atoptima
#
...
"""
function run!(context, ip_primal_sol; iter = 1)
...
is finished.
"""
function run_colgen_phase!(context, phase, stage, ip_primal_sol, stab; iter = 1)
    iteration = iter
    ...
    while !stop_colgen_phase(context, phase, colgen_iter_output, incumbent_dual_bound, ip_primal_sol, iteration)
        ...
        after_colgen_iteration(context, phase, stage, iteration, stab, ip_primal_sol, colgen_iter_output)
        iteration += 1
    end
...
function run_colgen_iteration!(context, phase, stage, ip_primal_sol, stab)
    ...
    if !isnothing(mast_primal_sol) && is_better_primal_sol(mast_primal_sol, ip_primal_sol)
        ...
        new_ip_primal_sol, new_cut_in_master = check_primal_ip_feasibility!(mast_primal_sol, context, phase)
