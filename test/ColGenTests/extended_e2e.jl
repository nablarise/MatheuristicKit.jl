...
struct ExtendedCallback end

# function RK.compute_reduced_costs(cb::ExtendedCallback, sol::MatheuristicKit.ColGen.DualMoiSolution)
#     return Dict{MOI.VariableIndex,Float64}()
# end


function MatheuristicKit.ColGen.variable_costs(callback::ExtendedCallback)
...
function test_e2e_extended_formulation()
    ...
    for (sp_id, sp_model) in ReformulationKit.subproblems(reformulation)
        ...
        RK.init_mapping_based_callback!(sp_model)

        sp_model.ext[:dw_colgen_callbacks] = ExtendedCallback()
    end
